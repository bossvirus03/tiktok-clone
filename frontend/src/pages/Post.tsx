...
import { GET_COMMENTS_BY_POST_ID } from "../graphql/queries/getCommentByPostId";
  ...
  return (
    ...
    >
      <div className="lg:w-[calc(100%-540px)] h-full relative">
        <Link to="/" className="absolute z-20 m-5 rounded-full hover:bg-gray-">
        ...
        {loadingPost ? (
          ...
          >
            <video
              ref={video}
              src={"http://localhost:6969" + dataPost.getPostById.video}
              loop
              muted
            ...
            {!isPlaying && (
              <AiFillPlayCircle
                size="100"
                className="text-gray-300  rounded-full z-100 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-blackcursor-pointer"
              />
            )}
          </div>
        )}
